...
**에이전트(Cursor, Claude Code 등)는 가능하면 `AGENTS.md`의 필수 읽기 순서·스택 판별·SaaS 화면 최소 구조를 먼저 따른다.**

**GitHub URL과 배포 문서 사이트 URL만 전달받은 경우:** 배포 사이트의 `/consume` 페이지(「링크로 시작 (AI)」)에 복사용 프롬프트와 필수 파일 링크가 정리되어 있다. 에이전트는 저장소의 `AGENTS.md`(본 문서와 동등한 절차)를 원격으로 읽을 수 있으면 최우선으로 읽는다.
...
| 파일 | 역할 |
|------|------|
| `AGENTS.md` | AI 실행 순서, 스택 자동 분기, SaaS 화면 최소 구조, 프롬프트 템플릿 |
| `docs/SEMANTIC_CONTRACT.md` | 시맨틱 표준 계약서 — 색상 시스템과 Button 소유권 |
| `docs/PRIMITIVE_CONTRACT.md` | primitive 표준 계약서 — Button 제외 KRDS 구조 기준 |
...
| `docs/PRODUCT_ADOPTION_GUIDE.md` | SaaS 제품 적용 가이드 (토큰 아키텍처, 반응형, Figma↔코드) |
...
| `docs/CONSISTENCY_AND_AUTOMATION.md` | AI/IDE/점검 스크립트까지 포함한 운영 규칙 |
| `src/data/figma-components.js` | Figma(KRDS) 컴포넌트·타이포·패턴 스펙 레퍼런스 |
| `src/data/principles.js` | 핵심 원칙 데이터 (문서 사이트·원칙 문구와 동기화) |
...
3. **표준 소유권** — `색상 + Button = HL`, `그 외 primitive 구조/크기/상태 = KRDS`를 따른다.`r`n4. **시맨틱 색상** — 색상은 의미로만 사용하라: 초록=정상, 노랑=경고, 빨강=에러, 파랑=선택/강조. 장식용 색상 금지.
5. **Compact Density** — 기본 밀도는 compact: 테이블 row 40px, 카드 gap 12px, 패딩 12~16px. Hero 섹션, 대형 여백 금지.
6. **다크 모드 필수** — 모든 UI는 라이트/다크 모드를 모두 지원하라. 순백/순검정 직접 사용 금지.
7. **Table First** — 모든 데이터는 테이블로 표현 가능해야 한다. 자유 시각화(차트, 3D 등)는 반드시 테이블/상세 패널과 연결하라.
8. **통일감 우선** — 10개의 서로 다른 도구를 열어도 같은 색상, 같은 구조, 같은 밀도가 되도록 하라.
9. **문서 우선** — GitHub의 마크다운·토큰·컴포넌트 코드가 기준이다. 배포 사이트는 시각·동작 참고용이며, 둘이 다르면 문서를 따른다.
10. **Identifier 선택 규칙** — SaaS처럼 푸터 정보가 많으면 `footer-out`, 푸터가 간결하면 `footer-in`을 사용하라.
