export const PRINCIPLES = [
  ...
  },
  {
    ...
    title: {
      ko: '타이포그래피 (한·영 공통)',
      en: 'Typography (Unified)',
    },
    summary: {
      ko: '본문·UI 산세리프는 한·영 모두 Pretendard Variable(`--hl-font`)로 통일하고, 숫자·코드는 Geist Mono(`--hl-mono`)를 씁니다.',
      en: 'Use Pretendard Variable (`--hl-font`) for all UI sans-serif (Korean and English). Use Geist Mono (`--hl-mono`) for numbers and code.',
    },
    detail: {
      ko: [
        '폰트 선택은 컴포넌트 내부에서 따로 처리하지 않고 전역 토큰으로 제어합니다.',
        '한·영 모두 동일 스택입니다: `Pretendard Variable` → 시스템 폰트. 숫자·코드·테이블 수치는 `Geist Mono`(`--hl-mono`)를 사용합니다.',
        'Figma(KRDS) 기준 타이포는 6계층입니다: Display(60/44/36 Bold), Heading(40/32/24/19/17/15 Bold), Body(19/17/15/13 Regular + Bold 변형), Label(19/17/15/13 Regular), Navigation(24/19 Bold + 17/15 Regular/Bold), Underline(19/17/15/13 Regular). 본문은 `Pretendard GOV`, 제목/강조는 `Pretendard Bold`입니다.',
        '개발에서는 위 스케일을 `--hl-text-*` 토큰에 대응시킵니다. Display·Heading은 프로젝트별로 가장 가까운 토큰(3xl/2xl/xl/lg/md/base/sm/xs)을 선택하고, Line-height는 leading 토큰(`--leading-tight(1.25)` ~ `--leading-relaxed(1.625)`)을 사용합니다.',
        '임의의 `font-size`, `letter-spacing`, `line-height`를 직접 지정하기보다 토큰을 재사용합니다. 상세 매핑은 `src/data/figma-components.js`의 `styles.typography`를 참조합니다.',
      ...
      en: [
        ...
        'Korean and English both use the same sans stack: `Pretendard Variable` with system UI fallbacks. Use `Geist Mono` (`--hl-mono`) for numeric and code-heavy areas.',
        'The Figma (KRDS) type system has 6 layers: Display (60/44/36 Bold), Heading (40/32/24/19/17/15 Bold), Body (19/17/15/13 Regular + Bold variants), Label (19/17/15/13 Regular), Navigation (24/19 Bold + 17/15 Regular/Bold), Underline (19/17/15/13 Regular). Body text uses `Pretendard GOV`; headings and emphasis use `Pretendard Bold`.',
        'In code, map these scales to the closest `--hl-text-*` tokens. For Line-height, use leading tokens (`--leading-tight(1.25)` through `--leading-relaxed(1.625)`). See `src/data/figma-components.js` → `styles.typography` for the full mapping.',
  ...
  },
  {
    ...
    detail: {
      ko: [
        ...
        'GitHub의 마크다운·토큰·컴포넌트 코드가 구현 기준이고, 배포 사이트는 시각·동작 참고용입니다. 둘이 다르면 GitHub 문서를 따릅니다.',
        'Identity(아이덴티티) 셸은 모든 페이지에 공통으로 적용되는 4개 영역입니다: Masthead(공식 배너) → Identifier(운영기관 식별자) → Header(헤더: 로고/슬로건 + Utility-small + Utility-medium 메뉴) → Footer(푸터: Related Site + 3-column 그리드 + 정책/저작권).',
        'Identity 영역의 본문은 `Pretendard GOV` Regular 15px, 메뉴/레이블은 `Pretendard` Bold 17px를 사용하며, 콘텐츠 영역 폭은 `1200px`, 좌우 여백은 `40px`입니다.',
        'Identifier는 SaaS처럼 푸터 정보가 많거나 복잡한 경우 footer-out을 우선하고, 푸터가 간결할 때만 footer-in을 사용합니다.',
        '새 제품이 셸을 구현할 때 위 4개 영역의 순서·폭·폰트·여백을 그대로 따르면 Identity 통일감이 유지됩니다.',
      ],
      en: [
        ...
        'GitHub markdown, tokens, and component code are the implementation authority; the deployed site is only a visual and behavior reference. If they conflict, follow the GitHub docs.',
        'The Identity shell consists of four common regions applied to every page: Masthead (official banner) → Identifier (agency identifier) → Header (logo/slogan + Utility-small + Utility-medium menu) → Footer (Related Site + 3-column grid + policies/copyright).',
        'Identity regions use `Pretendard GOV` Regular 15px for body text, `Pretendard` Bold 17px for menu/labels, a content area width of `1200px`, and horizontal padding of `40px`.',
        'For SaaS screens with dense or information-heavy footers, prefer the Identifier footer-out variant; use footer-in only when the footer is simple.',
        'When new products implement the shell, following the same order, width, font, and spacing of these four regions preserves Identity consistency.',
      ],
    },
  },
  {
    ...
    detail: {
      ko: [
        ...
        'Figma(KRDS v1.0.0) 기준 9개 카테고리(Identity/Navigation/Layout/Action/Select/Feedback/Help/Input/Setting) 935개 컴포넌트의 구조·치수·상태는 `src/data/figma-components.js`에 정리되어 있습니다. 새 컴포넌트 구현 시 이 파일을 레퍼런스로 참조합니다.',
      ],
      en: [
        ...
        'Structure, dimensions, and states for all 935 components across 9 categories (Identity/Navigation/Layout/Action/Select/Feedback/Help/Input/Setting) from Figma (KRDS v1.0.0) are cataloged in `src/data/figma-components.js`. Reference this file when implementing new components.',
      ],
    },
  },
];
